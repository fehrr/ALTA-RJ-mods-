...
pF.ready(async c => {
    ...
    if (!c.includes(e.table)) {
        await e.create(j => {
            j.id(), j.bigint('profile_id'), j.bigint('post_id').nullable(), j.varchar('image').nullable(), j.varchar('content').nullable(), j.varchar('comments').nullable(), j.bigint('created_at');
