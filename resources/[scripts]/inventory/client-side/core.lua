...
AddEventHandler("CEventGunShot",function(_,OtherPeds)
	local Ped = PlayerPedId()
	if Ped == OtherPeds and not LocalPlayer["state"]["Police"] and GetGameTimer() >= ShotDelay and Weapon ~= "WEAPON_MUSKET" and Weapon ~= "WEAPON_SAUER" then
...
AddEventHandler("inventory:Key",function()
	...
	if Keys["Vehicles"][1] then
		for _,v in pairs(Keys["Vehicles"]) do
			exports["dynamic"]:AddButton(VehicleName(v["vehicle"]).." - "..v["plate"],"Criar chave cópia por <green>$"..parseFormat(VehiclePrice(v["vehicle"])*0.02).."</green> Dólares.","inventory:Key",(VehiclePrice(v["vehicle"])*0.02).."-vehkey-"..v["plate"],"Vehicles",true)
		end
	...
	if Keys["Propertys"][1] then
		for _,v in pairs(Keys["Propertys"]) do
			...
			exports["dynamic"]:AddButton(Road.." Nº"..v.Number,"Criar chave cópia por <green>$"..parseFormat(1000).."</green> Dólares.","inventory:Key","1000-homekey-"..v.Keychain.."-"..v.Number,"Propertys",true)
		end
